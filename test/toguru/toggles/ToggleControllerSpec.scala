...
import toguru.toggles.ToggleActor.{CreateSucceeded, CreateToggleCommand, GetToggle}
...
class ToggleControllerSpec extends PlaySpec with Results with MockitoSugar {
  ...
  class MockToggleActor extends Actor {
    def receive: Receive = {
      case GetToggle               => sender ! Some(Toggle("toggle-id", "toggle", "desc", Map("team" -> "SharedServices")))
